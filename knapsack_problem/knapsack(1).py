...
def random_weights(num):
    return np.random.randint(1, high=MAX_WEIGHT, size=num)
...
def fchc_transpose_neighbour(x, neighborhood_size=5):
    old_v = calc_value(x) #calculate current value of sack
    n = random.sample(range(0, 4), 2) #generates 2 diff numbers 0 - 4

    for i in range(neighborhood_size): #try to get a better value this many times
