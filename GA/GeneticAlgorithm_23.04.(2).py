...
class Steady_State(Replacer):
    ...
    def replace(self, population, offspring, evaluationP, evaluationO):
        ...
        for i in range(self.number):
            minPos = evaluationP.index(min(evaluationP))
            evaluationP.pop(minPos)
            population.pop(minPos)

        for j in range(self.number):
            maxPos = evaluationO.index(max(evaluationO))
            evaluationO.pop(maxPos)
            population.append(offspring.pop(maxPos))
